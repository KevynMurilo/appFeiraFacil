...
export default function VerDetalhesByIdFeiranteAndBancaScreen() {
  const route = useRoute();
  const { feiranteId, bancaId, horarioId } = route.params;
  ...
  const buscarFeirante = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/feirantes/${feiranteId}/bancas/${bancaId}/horarios/${horarioId}`,
    ...
    return (
      <TouchableOpacity disabled={!isLink} onPress={handlePress} activeOpacity={0.7}>
        ...
        <View style={styles.linkContainer}>
          <Text style={[styles.valor, isLink && styles.linkTexto]}>{valor}</Text>
          {isLink && (
            <Ionicons
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <TopoNavegacao titulo="Feirante Detalhes" />
      <ScrollView
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.valor}>{feirante.bancas[0]?.tipoProduto || 'N/A'}</Text>

          <Text style={styles.label}>Produtos</Text>
          <View style={styles.listaProdutos}>
            {feirante.bancas[0]?.produtos?.map((prod, idx) => (
          ...
          {feirante.bancas[0]?.horarios?.map((h, idx) => (
            <View key={idx} style={styles.horarioBox}>
              <Text style={styles.valor}>
